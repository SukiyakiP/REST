...
class RESTInferenceApp:
    def __init__(self, root):
        self.root = root
        self.root.title("REST Inference GUI (CPU)")
    ...
    def _take_screenshot(self, _event=None):
        try:
            ...
            # Grab the specific pixel area of the window
            img = ImageGrab.grab(bbox=(x, y, x + w, y + h))

            # Upscale 3x with high quality resampling to satisfy physical space 300dpi requirements
            new_size = (img.width * 3, img.height * 3)
            img = img.resize(new_size, Image.Resampling.LANCZOS)

            # Save strictly as 300 DPI
            save_path = "GUI_Screenshot_300dpi.png"
            img.save(save_path, dpi=(300, 300))

            messagebox.showinfo("Screenshot Saved", f"High-Resolution 300dpi Screenshot saved to:\n\n{save_path}")
    ...
    def _run_single_inference(self, filepath, ch_eeg1, ch_emg, use_hmm, use_artifact_filter):
        try:
            ...
            power = compute_powers_welch_tensor(eeg_data * 1e6, emg_data * 1e6, sfreq=FS, s=EPOCH_LENGTH, device=DEVICE)
